...
class TestResponseStructure:
    ...
    def test_book_response_structure(self, client, test_book):
        ...
        required_fields = ["id", "title", "author", "description", 
    ...
    def test_catalog_response_structure(self, client, test_book):
        ...
        if isinstance(data, list):
            ...
            if len(data) > 0:
                first_book = data[0]
                required_fields = ["id", "title", "author", "cover_image_uri", 
                                  "readers_count", "tags"]
                for field in required_fields:
                    assert field in first_book, f"Field '{field}' not found in catalog item"
        else:
