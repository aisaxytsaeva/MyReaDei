...
class TestTags:
    ...
    def test_get_all_tags(self, db_session, test_moderator):
        tags_crud.create_tag(
            db_session,
            CreateTag(tag_name="Тег1", description="Описание1"),
            user_id=test_moderator.id
        )
        tags_crud.create_tag(
            db_session,
            CreateTag(tag_name="Тег2", description="Описание2"),
            user_id=test_moderator.id
        )
    ...
    def test_update_tag(self, db_session, test_moderator):
        tag = tags_crud.create_tag(
            db_session,
            CreateTag(tag_name="Старое", description="Старое описание"),
            user_id=test_moderator.id
        )
        ...
        try:
            ...
            updated = tags_crud.update_tag(
                db_session, tag.id, TagUpdate(tag_name="Новое")
            )
    ...
    def test_update_tag_description(self, db_session, test_moderator):
        tag = tags_crud.create_tag(
            db_session,
            CreateTag(tag_name="Тег", description="Старое описание"),
            user_id=test_moderator.id
        )

        try:
            updated = tags_crud.update_tag(
                db_session, tag.id, description="Новое описание"
            )
        except TypeError:
            updated = tags_crud.update_tag(
                db_session, tag.id, TagUpdate(description="Новое описание")
            )
    ...
    def test_delete_tag(self, db_session, test_moderator):
        tag = tags_crud.create_tag(
            db_session,
            CreateTag(tag_name="На удаление", description="Будет удален"),
            user_id=test_moderator.id
        )

        try:
            result = tags_crud.delete_tag(
                db_session, tag.id, user_id=test_moderator.id
            )
        except TypeError:
